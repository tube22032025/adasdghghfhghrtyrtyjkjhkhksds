...
uninstall_3xui() {
    log_warn "Kiểm tra và gỡ bỏ 3x-ui cũ nếu có..."
    
    # Tắt set -e tạm thời để tránh exit sớm
    set +e
    ...
    rm -rf /usr/local/x-ui /etc/x-ui 2>/dev/null
    rm -f /usr/bin/x-ui 2>/dev/null
    
    # Bật lại set -e
    set -e
    
    log_info "Đã hoàn tất gỡ bỏ 3x-ui cũ (nếu có)"
}
...
main() {
    ...
    PANEL_PASSWORD=$(echo "$INSTALL_OUTPUT" | grep -iE "^\s*Password:" | grep -vi "Root" | tail -1 | sed 's/.*Password:[[:space:]]*//' | tr -d '[:space:]')
    [[ -z "$PANEL_PASSWORD" ]] && PANEL_PASSWORD="N/A"
    
    # Lấy Port (dòng chứa "Port:")
    PANEL_PORT=$(echo "$INSTALL_OUTPUT" | grep -iE "^\s*Port:" | tail -1 | sed 's/.*Port:[[:space:]]*//' | tr -d '[:space:]')
